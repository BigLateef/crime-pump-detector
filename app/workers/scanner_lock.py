...
async def acquire_lock() -> str | None:
    ...
    Retries once on a transient Redis error before giving up and raising
    ScannerLockUnavailable - the underlying client (see
    app/core/redis_client.py) already retries transparently on most
    connection blips via retry_on_error, but this adds one more explicit
    attempt at this layer specifically because failing to acquire the
    lock here means skipping an entire scan cycle, which is a much more
    expensive thing to give up on than a single cache read elsewhere in
    the app - worth one extra real attempt before doing that.
    """
    ...
    for attempt in range(2):
        try:
            ...
            last_error = e
            logger.warning(
                "scanner lock acquire attempt %d failed: Redis unreachable (%s)%s",
                attempt + 1, type(e).__name__, " - retrying once" if attempt == 0 else "",
            )
    logger.error("cannot acquire scanner lock after retry: Redis unreachable (%s)", type(last_error).__name__)
    raise ScannerLockUnavailable(str(last_error)) from last_error
