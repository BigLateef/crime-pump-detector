...
class Settings(BaseSettings):
    ...
    # When true (and DATA_PROVIDER_MODE=live), this becomes the discovery
    # source instead of DexScreener - DexScreener's free tier has no
    # "newest pairs" endpoint at all (see dexscreener.py's
    # discover_new_pairs, which deliberately raises NotImplementedError
    # rather than fake one), so without this the scanner discovers
    # nothing regardless of live mode being on. See app/adapters/
    # geckoterminal.py's module docstring for the full picture, including
    # a real capability GeckoTerminal has that DexScreener's free tier
    # doesn't: genuine unique buyer/seller counts.
    geckoterminal_enabled: bool = False
